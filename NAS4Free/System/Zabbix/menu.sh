...
do
        echo -e "${sep}"
        echo -e "${fin} TEMPLATE${nc}"
        echo -e "${sep}"
        echo -e "${qry} Choose one:${nc}"
        echo " "
        echo -e "${fin}   1)${msg} Install TEMPLATE${nc}"
        echo -e "${fin}   2)${msg} Update TEMPLATE${nc}"
        echo -e "${ca}   3)${ca} Backup (Currently Unavailable)${nc}"
        echo " "
        echo -e "${inf}  a) About TEMPLATE${nc}"
        ...
        case $choice in
            '1')	printf '\033\143'; echo -e "${inf} Please confirm that you wish to install TEMPLATE${nc}" ; echo " "
					. $scriptPath/TEMPLATE/install.sh ;;
            '2') 	printf '\033\143'; echo -e "${inf} Running Update..${nc}" ; echo " "
					. $scriptPath/Databases/TEMPLATE/update.sh ;;
            '3')	printf '\033\143'; echo -e "${inf} Backup..${nc}" ; echo " "
					. $scriptPath/Databases/TEMPLATE/backup.sh ;;

            'a')	printf '\033\143'; . $scriptPath/TEMPLATE/about.sh ;;
            'h')	printf '\033\143'; (. $scriptPath/gethelp.sh);;
            'i')	printf '\033\143'; . $scriptPath/TEMPLATE/moreinfo.sh ;;
